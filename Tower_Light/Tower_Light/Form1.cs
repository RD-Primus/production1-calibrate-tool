    ...
    public partial class Form1 : Form
    {
        ...
        private async void Test1_btn_Click(object sender, EventArgs e)
        {
            ...
            for (int relay = 0; relay < activeTiers; relay++)
            {
                ...
                {
                    allPass = false;
                    staticErrors.Add($"[Static Test] Relay {relay + 1} ({cName}) ADC {peakAdc[relay]} < Min {minAcceptable[relay]}");
            ...
            if (IsFlashEnabled)
            {
                ...
                for (ushort relay = 0; relay < activeTiers; relay++)
                {
                    ...
                    else
                    {
                        ...
                        allBlinkPass = false;
                        blinkErrors.Add($"[Blink Test] Relay {relay + 1} ({cName}) ความถี่ {hz:F2} Hz (นอกเกณฑ์ 0.75-0.95)");
            ...
            {
                _goodCount++;
                _failReason = "All PASS";
                AddLog($"Tower Light serial {sn_txt.Text} นี้ผ่านเรียบร้อย", Color.LimeGreen);
                MessageBox.Show("การทดสอบสำเร็จผ่านทุกขั้นตอน!", "Test Complete", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                _defectCount++;
                ...
                if (IsBuzzerEnabled && !soundPass) { displayErrors.Add("[Sound Test] ค่าไม่อยู่ในเกณฑ์"); }

                _failReason = "Fail";
                AddLog($"❌ Tower Light serial {sn_txt.Text} ไม่ผ่านการทดสอบ", Color.Red);
                MessageBox.Show("สรุปผลการทดสอบ: พบปัญหาดังนี้\n\n" + string.Join("\n", displayErrors), "Test Failed", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            ...
            for (int retry = 0; retry < maxRetries; retry++)
            {
                try
                {
                    using (var workbook = new XLWorkbook(_excelSavePath))
                    {
                        ...
                        worksheet.Cell("A6").Value = chk3Colors.Checked ? "☑ มีให้เลือกมากถึง 3 สี คือ เขียว,เหลือง , แดง" : "☐ มีให้เลือกมากถึง 3 สี คือ เขียว,เหลือง , แดง";
                        worksheet.Cell("A7").Value = chkSwapColors.Checked ? "☑ สามารถสลับสีของไฟได้เอง ตามการใช้งาน" : "☐ สามารถสลับสีของไฟได้เอง ตามการใช้งาน";
                        worksheet.Cell("A8").Value = chkContinuousBlink.Checked ? "☑ การทำงานแบบติดต่อเนื่องและกระพริบ" : "☐ การทำงานแบบติดต่อเนื่องและกระพริบ";
                        worksheet.Cell("H5").Value = chkShockTest.Checked ? "☑ การทำงานเมื่อมีการ Shock Test" : "☐ การทำงานเมื่อมีการ Shock Test";
                        worksheet.Cell("H6").Value = chkBuzzer.Checked ? "☑ การทำงานเสียงเตือนของ Busser (90db Up)" : "☐ การทำงานเสียงเตือนของ Busser (90db Up)";
                        worksheet.Cell("H7").Value = chkLED360.Checked ? "☑ ใช้หลอด LED สามารถส่องเห็นได้ชัดเจน 360'" : "☐ ใช้หลอด LED สามารถส่องเห็นได้ชัดเจน 360'";
                        worksheet.Cell("H8").Value = chkSupply.Checked ? "☑ Supply 100-240 VAC" : "☐ Supply 100-240 VAC";
                        worksheet.Cell("M6").Value = chkCE.Checked ? "☑ ตรวจสอบเครื่องหมาย CE" : "☐ ตรวจสอบเครื่องหมาย CE";
                        worksheet.Cell("M7").Value = chkCurrent.Checked ? "☑ 21.35 mA" : "☐ 21.35 mA";
                        worksheet.Cell("M8").Value = chkECN.Checked ? "☑ ECN NO:RD-CN-25-3-0061" : "☐ ECN NO:RD-CN-25-3-0061";
